...
def printLocalAUC():
    ...
    print "Z:"
    print Z
    print (accu+0.0)/(m*n*n)
    return (accu+0.0)/(m*n*n)
...
getNewV()
count = 0
distanceU = distance_matrix(U_new, U)
distanceV = distance_matrix(V_new, V)
...
or abs(distanceV - newDistanceV) > precision):
    ...
    newDistanceU = distance_matrix(U_new, U)
    newDistanceV = distance_matrix(V_new, V)
